...
def intersession(df, save_path_intersesion):
    ...
    if df.shape[0] > 1:
        ...
        with PdfPages(save_path_intersesion) as pdf:
            ...
            subset = df.loc[((df['trial_result'] == 'correct_first') | (df['trial_result'] == 'correct_other') | (
                        df['trial_result'] == 'punish'))]

            axes = plt.subplot2grid((50, 50), (6, 27), rowspan=5, colspan=24)
            ymax = subset.lick_latency.median() + 10
            sns.lineplot(x='day', y='lick_latency', hue='trial_result', hue_order=treslt_order, style='stage',
                         data=subset,
            ...
            axes.get_legend().remove()
            lines = [
                Line2D([0], [0], color=treslt_palette[i], marker='o', markersize=7, markerfacecolor=treslt_palette[i])
            ...
            sns.lineplot(x='day', y='correct_bool', hue='trial_type_simple', hue_order=ttype_order,
                         palette=ttype_palette,
            ...
            # legend
            lines = [
                Line2D([0], [0], color=ttype_palette[i], marker='o', markersize=7, markerfacecolor=ttype_palette[i])
